...
export async function connectMongo() { await mongoose.connect(config.mongoUrl); }
export async function connectRedis() { redis = createClient({ url: config.redisUrl }); redis.on('error', console.error); await redis.connect(); }
